...
- Release는 STAGED 검증을 통과한 REGISTERED bundle만 사용합니다. staging은 `.env` 변형, private key, VCS metadata를 거부합니다. `v0.6.1`의 공식 commit은 `aac850769e97388884e49c0068ea97f691e06d9e`이고 원격에 없는 `v0.6.0`은 baseline으로 등록하지 않습니다. runner는 bundle의 app import를 현재 checkout과 격리하지만 현재 Python interpreter와 site-packages를 사용하므로 dependency-hermetic 실행은 아닙니다.
...
- 로컬 bytes가 없거나 손상·비호환이면 과거 identity를 바꾸지 않고 새 실행만 차단합니다. 같은 digest의 exact bytes 복원만 기존 자산의 가용성을 회복합니다.
