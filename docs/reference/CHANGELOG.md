...
- `아모레퍼시픽`의 `모레`, `전주페이퍼`의 `전주`처럼 기업명 일부를 상대 날짜로 인식하던 문제를 수정했습니다. 단어 경계와 한국어 조사·보고서 표현을 구분해 날짜 조건을 판별합니다.
- 날짜를 새로 지정하지 않은 기업별 후속 질문에서 이전 조회 기간이 유지되도록 했습니다. `모레에`, `오늘자`, `이번주에발간된` 등 실제 날짜 표현도 계속 지원합니다.

## v0.6.4 - 2026-09-12

### Fixed

- 단일 종목 RDB 조회에서 LLM이 작성한 SQL의 대상 조건을 검증하지 않던 문제를 수정했습니다. 대상 조건을 결정적으로 바인딩하고, 범위를 확정할 수 없으면 조회하지 않고 안내합니다.
- 조회 결과에 `Error:` 문자열이 포함되면 정상 결과를 오류로 오인해 답변 전체를 폐기하던 문제를 수정했습니다. 가드레일 차단과 실행 실패를 문자열이 아닌 예외로 구분합니다.
- 리랭커를 켠 단일 검색 경로에서 리랭크 실패가 응답 전체를 중단시키던 문제를 수정했습니다. 비교 경로와 같이 검색 순서로 대체합니다.
- 후보 빌드 evidence를 원자적으로 기록하도록 수정했습니다. 기록 중단 시 이후 재시도가 영구 실패하던 문제를 없앴습니다.
- Release 등록 시 git 출력을 UTF-8로 디코드하도록 수정했습니다. 한국어 Windows에서 인코딩 오류로 등록이 중단되던 문제를 해결했습니다.

### Changed

- RDB 조회 실패 안내를 원인별로 구분했습니다. 조회 범위를 확정하지 못한 경우 별도 안내 문구를 사용합니다.
- 개별 Chat 상세의 RDB 지표에 `scope_rejected`, `error`를, 리랭크 저하 시 `rerank_degraded`를 추가했습니다.

## v0.6.3 - 2026-09-05

### Fixed

- 2~5개 대상을 비교하는 답변에서 참고 문서 인용 번호가 청크(조각) 단위로 여러 개 중복 표시되던 문제를 문서 단위 인용으로 정규화했습니다.
- `SFA반도체`처럼 영문접두사를 가진 target이 `반도체` 별칭을 만들어, 산업 target `반도체`가 검색 범위에서 누락되던 문제를 수정했습니다. 별칭이 여러 target에 걸릴 때 전체 이름과 정확히 일치하는 target을 우선합니다.
...
- 답변 인용이 실제로 참조한 PDF 리포트와 일치하도록 검색·응답·모니터링 근거 연결을 보강했습니다.
- 증분 임베딩 과정에서 정상 PDF가 잘못 거부될 수 있는 경계를 수정했습니다.

### Changed

- 현재 Native V2 실행 경로와 맞지 않는 오래된 Monitoring·설정 표면을 정리하고 관련 문서를 현재 동작에 맞췄습니다.
