...
*   **PDF 파싱(구조적 텍스트 추출):** `PyMuPDF4LLM` (기본) + `marker-pdf` (선택 옵션)
    *   *이유:* 증권사 리포트는 구조가 복잡하고 텍스트 레이아웃 정보가 중요한데, `PyMuPDF4LLM`은 제목(#) 구조를 잘 잡아내며 매우 빠릅니다. 반면, 딥러닝 기반의 `marker-pdf`는 GPU 환경에서 표와 수식을 더 정밀하게 추출할 수 있어 사용자가 환경에 맞춰 선택할 수 있도록 설계했습니다. (최종 폴백으로 PyMuPDF `fitz` 사용)
...
3. **Markdown 중심의 지능형 텍스트 처리:**
   - **Step 3.1. 엔진별 추출 (PDF → Markdown):** 설정(`config.py`)에 따라 `PyMuPDF4LLM`(기본) 또는 `marker-pdf`(옵션)를 사용하여 PDF를 마크다운으로 변환합니다. 이때 폰트 크기와 굵기를 분석해 `# 제목` 구조를 생성합니다. (추출 실패 시 표준 `fitz` 모드로 자동 폴백)
