...
| 항목 | 값 |
| --- | --- |
| 상태 | 운영판 기획 초안 · 이슈 접수 서버/클라이언트 1단계 hosted 검증 완료 · 운영자 화면 미구현 |
| 문서 버전 | 0.1.0 |
| 기획 책임자 | 기획자 / 운영자 |
| 기술 검토자 | 지정 필요 |
| 최종 갱신일 | 2026-08-09 |

> 이 문서는 별도로 개발할 운영판의 목표 경계를 설명한다. 현재 저장소에는 Supabase 익명 이슈 접수의 Edge Function·DDL·입력 검증과 로컬 SQLite outbox·비동기 재시도 클라이언트가 구현되어 있고, hosted 배포·권한 smoke test와 보존 Cron 검증까지 완료됐다. 일반 관측 이벤트와 운영자 원격 화면은 아직 구현되지 않았다.
