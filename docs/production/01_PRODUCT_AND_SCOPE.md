...
4. 앱은 즉시 신고 접수 완료를 안내하고 원격 전송 상태는 사용자에게 노출하지 않는다.
5. 별도 로컬 신고 산출물은 만들지 않고 bounded 원격 payload만 retry-only outbox에 넣어 background worker에서 전송한다.
6. 영구 거절 또는 재시도 만료는 조용히 실패 처리하고 해당 outbox payload를 삭제한다.
7. 운영자는 원격 이슈를 triage하되, 기대 결과 승인 전에는 평가 사례로 승격하지 않는다.
